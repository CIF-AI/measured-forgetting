...
In practice, we observed reliable degradation after 4--5 tool rounds on Qwen3 8B (8K context) and 6--8 rounds on the same model at 16K context. The failure mode is not a crash --- it is \emph{confabulation}. The model starts generating plausible-sounding responses that ignore the tool results, because the relevant data has been pushed too far back in the context.
...
if reused < *n_past as usize {
    // Prompt diverged. M-RoPE models (e.g. Qwen) require
    // strictly increasing positions, so a partial trim
    // corrupts position state -- clear the whole cache.
    ctx.clear_kv_cache();
    *n_past = 0;
}
\end{lstlisting}
...
\begin{enumerate}
  ...
  \item \textbf{Divergence} ($\texttt{reused} < n_{\text{past}}$): The new prompt diverges from the cached version. Because the deployed models use M-RoPE (which requires strictly increasing KV positions), a partial trim would corrupt position state; the cache is therefore cleared in full and the new prompt re-decoded from scratch. This is the case that fires after compaction restructures the message history --- compaction trades a one-time full re-decode for a fresh context budget.
...
\begin{enumerate}
  ...
  \item When context crosses the compaction threshold ($\frac{3}{4}$ of the usable budget $C-G$), measured forgetting compresses the middle zone.
  \item The compression rewrites the message history below the pinned prefix; because M-RoPE forbids a partial KV trim, the cache is cleared and the (now much shorter) restructured prompt is re-decoded once.
...
After compression, the model is asked each probe question and the answer is scored 0--3 (0 = wrong/unknown, 1 = partially correct, 2 = correct but vague, 3 = correct with specifics) via keyword matching against predefined required and partial keywords. This automated scoring is intentionally coarse-grained: it rewards factual recall (proper nouns, numbers, specific terms) rather than nuanced understanding, and will undercount correct paraphrases that use synonyms. At publication we accepted this trade-off on two grounds: (a)~the same scorer is applied uniformly to all three conditions, so any bias should cancel in the V2$-$V1 comparison, and (b)~keyword presence/absence is reproducible and deterministic, unlike LLM-as-judge scoring which introduces model-dependent variance. \textbf{Correction (v2.1):} ground (a) does not hold --- cancellation requires the scorer's bias to be \emph{independent of the treatment}, but V2's primary mechanism (preserve entities and numbers verbatim) is collinear with exactly what a keyword scorer rewards. The V2$-$V1 deltas in Tables~\ref{tab:overall}--\ref{tab:dim} should therefore be read as \emph{upper bounds} confounded by scorer--mechanism collinearity; Section~\ref{sec:corrections} details the reassessment and the replacement scorer.
...
Phi-4 mini 3.8B & Microsoft & 2.02 & 2.67 & \textbf{2.94} & +0.27 \\
...
\midrule
\textbf{Mean} & & \textbf{1.69} & \textbf{2.37} & \textbf{2.59} & \textbf{+0.22} \\
\bottomrule
\end{tabular}
\caption{Retention scores (out of 3.0) across 7 models from 5 independent families, 18 scenarios, 48 probes per model. V2 outperforms V1 on all 7 models with zero regressions. Sorted by V2$-$V1 delta.}
\label{tab:overall}
\end{table}

\noindent V2 outperforms V1 on every model tested (sign test: 7/7 positive, $p = 0.008$). The mean improvement is +0.22 (9\% relative), with the largest gain on the weakest summariser (+0.46 on Qwen3 8B, a 131\% relative improvement). Directional evidence suggests an inverse capability relationship ($r = -0.93$, $p = 0.003$, $n=7$): dimension-aware compression provides the largest benefit precisely where uniform summarisation is weakest. We note that $n=7$ is too small for robust correlation claims; the sign test (which requires only direction, not magnitude) provides the stronger statistical grounding.
...
\midrule
Lookup & 1.26 & 2.31 & \textbf{2.90} & Influence identifies the answer \\
Multi-hop & 1.10 & 1.97 & \textbf{2.52} & Chain detection preserves path \\
Temporal & 1.41 & 2.35 & \textbf{2.52} & Dimension scoring preserves dates \\
Exploratory & 1.96 & 2.54 & \textbf{2.54} & V2 matches V1 (already strong) \\
Aggregation & 1.94 & \textbf{2.60} & 2.56 & V1 edges V2 (single-dim data) \\
Contradiction & 2.55 & 2.47 & \textbf{2.59} & Dual-path preservation helps \\
\bottomrule
\end{tabular}
\caption{Retention score by problem class (mean across all 7 models). V2's largest gains are on structured problems: lookup (+0.59 over V1) and multi-hop (+0.55), with a smaller gain on temporal (+0.17). The advantage vanishes on exploratory (tied) and slightly reverses on aggregation ($-0.04$), where V1 already captures the sufficient statistic.}
\label{tab:class}
\end{table}
...
\midrule
Entity recall & 1.21 & 1.21 & \textbf{3.00} \\
Fact retention & 1.68 & 2.46 & \textbf{2.66} \\
Contradiction awareness & 2.24 & 2.33 & \textbf{2.57} \\
Temporal ordering & 1.11 & 2.23 & \textbf{2.31} \\
Causal fidelity & 1.89 & 2.44 & \textbf{2.49} \\
\bottomrule
\end{tabular}
\caption{Retention score by probe dimension (mean across 7 models). V2 achieves a perfect score on entity recall (3.00 across all 7 models) and wins on every dimension. The preserved-verbatim tier is the primary mechanism: proper nouns and conflicting data points score high on the Influence Equation and are kept intact.}
\label{tab:dim}
\end{table}
...
\textbf{Aggregation parity:} V2 scores 2.56 vs V1's 2.60 on aggregation --- the only class where V2 does not win. Aggregation data hits a single dimension (data), preventing the super-linear $\Phi$ boost. At publication we proposed a class-specific override as the natural fix. \textbf{Correction (v2.1):} the override patches the proxy instead of measuring the quantity. By the two-axis analysis of Section~\ref{sec:corrections}, a numbers-dense single-dimension message is \emph{maximal} on both real signals --- high surprisal (raw data the model cannot infer) and high withholding-degradation (drop it and the answer breaks) --- and the lexical proxy fails precisely where both real signals fire. The fix is the removal-$\Delta$ scorer, not another keyword rule.

\textbf{Causal fidelity ceiling:} V2 improves causal fidelity only marginally over V1 (2.49 vs 2.44). The ``why'' explanation often lives in assistant messages with low multi-dimensional scores. Semantic dependency detection (beyond structural tool-result $\to$ interpretation links) remains an open problem.
...
% ══════════════════════════════════════════════════════════════════════
\section{Corrections and Reassessment (v2.1)}
\label{sec:corrections}

Subsequent analysis --- grading this paper against a two-axis framework of memory value (encoding worth: what is worth writing; retrieval timing: what is worth keeping), and a production deployment of the corrected instrument in a graph memory system (cif-graph, 2026-07-08) --- surfaced errors that individual review had treated as separate weaknesses. They collapse into one: \emph{the influence equation and the benchmark scorer both measure the expression layer (lexical surface) to decide what carries meaning}. This section records the corrections; a re-benchmark with the corrected scorer is in progress.

\subsection{The scorer measures token-survival, not meaning-survival}

Table~\ref{tab:dim}'s headline row --- entity recall 1.21 $\to$ 3.00, a \emph{perfect} score across all 7 models --- is the diagnostic signature, not the achievement we presented it as. V2's preserved-verbatim tier keeps the exact string the grader greps for; a perfect score under those conditions is near-tautological. The ``bias cancels in V2$-$V1'' defence fails because the bias is not independent of the treatment (Section 5.4, corrected above). The scorer is additionally dimension-blind in one tested ability: a summary that lists both conflicting numbers while asserting they \emph{agree} still scores full marks on ``contradiction awareness'' --- the probe checks keyword presence, not the contradiction judgment.

\subsection{$\Phi(B) = B^{1.5}$ is an unwitting proxy for a measurable quantity}

We justified the super-linear multiplier by arguing that ``multi-dimensional convergence is harder to reconstruct from a summary.'' That phrase \emph{is} the withholding test: a stored item's true keep-value is revealed by removing it and measuring whether downstream performance degrades. $\Phi$ approximates reconstructability by \emph{counting dimensions} instead of running the removal. The constant $1.5$ was selected by tier-balance on the 18 evaluation scenarios --- a hand-tuned routing threshold; the name ``super-linear resonance'' outran the evidence. The principled replacement scores each candidate message $m$ directly:
\begin{equation}
\Delta(m) \;=\; \mathrm{Perf}(\text{context}) \;-\; \mathrm{Perf}(\text{context} \setminus m)
\label{eq:delta}
\end{equation}
$\Delta$ \emph{is} the keep-value: preserve high-$\Delta$ verbatim, summarise low-$\Delta$, drop near-zero-$\Delta$ (reconstructable from what remains). Unlike dimension-counting, Equation~\ref{eq:delta} can also \emph{disconfirm} $\Phi$ --- the ablation v2.0 could not run.

\subsection{The retrieval axis was absent}

Measured forgetting v2.0 had no notion of reconstructability: it never asked ``can the model rebuild this from what remains?'' --- which is the entire game of lossy compression. Everything reconstructable is free to drop; everything load-bearing must stay, \emph{regardless of its lexical shape}. Susceptibility decay reached toward this (down-weight the superseded) but implemented it as token-category bookkeeping rather than measured predictive redundancy.

\subsection{Production evidence for the corrected instrument}

The withholding probe of Equation~\ref{eq:delta} is deployed in a graph memory system (cif-graph): for at-risk memories, an LLM reconstructs the withheld item from its graph neighborhood (Path~B), and reconstruction quality is scored two independent ways --- embedding cosine (an expression-layer measure, like this paper's scorer) and a calibrated LLM judge with known-answer controls (identical pair should score $\sim$100 preserved; faithful paraphrase should score high). First 60 production probes (2026-07-08): the expression-layer scorer reported mean similarity $0.85$ (``mostly preserved''), while the meaning judge scored preservation near zero and flagged \emph{half} of the judged reconstructions as asserting something the original contradicts. A neighborhood-centroid baseline sided with the judge (reconstructions were no closer to the original than generic context). This is the failure mode of expression-layer scoring observed live, at scale, in a system built by the same authors: the cheap scorer overestimates retention exactly when the content is specific --- and specificity is what retention is \emph{for}.

\subsection{Re-benchmark design}

The v2.1 re-benchmark (in progress) replaces the instrument rather than re-arguing the result: (1)~keyword scoring is replaced by a calibrated LLM judge with identical/paraphrase control brackets reported alongside every run --- the determinism objection is answered by \emph{measuring} judge variance instead of avoiding it; (2)~$\Delta$-ranking (Equation~\ref{eq:delta}) replaces $\Phi$ as the influence signal, with $\Phi$ retained as an ablation arm so the dimension-counting hypothesis is finally testable; (3)~results report the per-(model $\times$ scenario) worst case, not only class means --- v2.0 reported aggregation's mean regression but no cell-level floor; (4)~the Qwen3 outlier that anchors the inverse-capability claim is re-tested under $\Delta$-ranking, since patterns~1 (weak models gain most) and~3 (thinking-token artifact) are in tension as published. Claims in Sections 5.4--5.6 should be treated as provisional until this re-benchmark reports.

\subsection{The companion axis: measured appreciation}

Forgetting decides what to release; its inverse decides what to protect. In the production system the same withholding measurement quantifies \emph{appreciation}: a memory whose surrounding context can no longer reconstruct it --- and, in the strongest case, now implies the \emph{opposite} of it --- has grown more valuable as its neighborhood decayed, the way scarcity appreciates an asset. Measured forgetting and measured appreciation are the two directions of one instrument: $\Delta$ near zero licenses compression; $\Delta$ large (or contradiction-flagged) licenses protection. A memory system that implements only the forgetting direction, as v2.0 did, will eventually compress its treasures.
...
\begin{tikzpicture}[
  ...
  \node[font=\footnotesize, align=left] at (5.5, -0.1) {
    ...
    6.~If ctx $> T_{\text{compact}}$: \textsc{Compact}\\
    7.~Goto 2
  };
  ...
  % llama.cpp
  \node[box, fill=purple!8] (llama) at (5.5, -6.0) {llama.cpp + Metal\\[-2pt]\footnotesize Qwen3 8B Q4\_K\_M};
...
The following is an illustrative trace --- representative of B.app (Qwen3 8B, M4 Mac) behaviour during a multi-tool investigation, with token counts rounded for exposition. The user asked: ``What's the current price of GBPUSD and what was the 24h change?'' The model dispatched tool calls to a forex API.
...
----- context: 10,910 / 16,384; exceeds T_compact = 10,752 -----
----- COMPACTION FIRES -----
\end{verbatim}
\end{small}

\noindent Once context crosses the compaction threshold $T_{\text{compact}} = \frac{3}{4}(C-G)$ (Eq.~\ref{eq:threshold}), compaction fires. The v2 algorithm analyses the 5 intermediate messages:
...
\noindent The compacted context is 3,200 tokens (from 10,910) --- a 71\% reduction. The model continues with a fresh budget; because the prompt diverged at the compression point, this one compaction triggers a full re-decode of the (now much shorter) restructured prompt, after which KV persistence resumes on subsequent rounds.
